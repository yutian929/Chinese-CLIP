    ...
    with open(submit_path, encoding="utf-8") as fin:
        for line in fin:
            ...
            if "image_id" not in pred_obj:
                raise Exception(
                    "There exists one line not containing image_id: {}".format(line)
                )
            ...
            if "text_ids" not in pred_obj:
                raise Exception(
                    "There exists one line not containing the predicted text_ids: {}".format(
                        line
                    )
                )
            ...
            if not isinstance(text_ids, list):
                raise Exception(
                    "The text_ids field of image_id {} is not a list, please check your schema".format(
                        image_id
                    )
                )
            ...
            if len(set(text_ids)) != k:
                raise Exception(
                    "Image_id {} has duplicate topk texts in your prediction. Pleace check again!".format(
                        image_id
                    )
                )
