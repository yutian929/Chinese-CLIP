...
class EvalTxtDataset(Dataset):
    def __init__(self, jsonl_filename, max_txt_length=24):
        assert os.path.exists(
            jsonl_filename
        ), "The annotation datafile {} not exists!".format(jsonl_filename)
    ...
    def __getitem__(self, idx):
        ...
        text = tokenize(
            [_preprocess_text(str(text))], context_length=self.max_txt_length
        )[0]
        return text_id, text
...
class EvalImgDataset(Dataset):
    def __init__(self, lmdb_imgs, resolution=224):
        assert os.path.isdir(
            lmdb_imgs
        ), "The image LMDB directory {} not exists!".format(lmdb_imgs)
        ...
        logging.info(
            "The specified LMDB directory contains {} images.".format(
                self.number_images
            )
        )
...
def get_eval_img_dataset(args):
    ...
    dataset = EvalImgDataset(lmdb_imgs, resolution=fetch_resolution(args.vision_model))
    num_samples = len(dataset)
    sampler = SequentialSampler(dataset)
