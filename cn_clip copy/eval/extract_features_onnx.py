...
if __name__ == "__main__":
    ...
    img_session = onnxruntime.InferenceSession(
        args.onnx_image_model, sess_options=img_sess_options, providers=[provider]
    )
    ...
    txt_session = onnxruntime.InferenceSession(
        args.onnx_text_model, sess_options=txt_sess_options, providers=[provider]
    )
        ...
        with open(args.text_feat_output_path, "w") as fout:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    ...
                    text_features = txt_session.run(
                        ["unnorm_text_features"], {"text": texts.numpy()}
                    )[0]
                    ...
                    for text_id, text_feature in zip(
                        ...
                        fout.write(
                            "{}\n".format(
                                json.dumps(
                                    {"text_id": text_id, "feature": text_feature}
                                )
                            )
                        )
                        write_cnt += 1
        ...
        with open(args.image_feat_output_path, "w") as fout:
            ...
            with torch.no_grad():
                for batch in tqdm(dataloader):
                    ...
                    image_features = img_session.run(
                        ["unnorm_image_features"], {"image": images.numpy()}
                    )[0]
                    ...
                    for image_id, image_feature in zip(
                        ...
                        fout.write(
                            "{}\n".format(
                                json.dumps(
                                    {"image_id": image_id, "feature": image_feature}
                                )
                            )
                        )
                        write_cnt += 1
