...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of workers for ImageNet dataloader.",
    )
...
def zero_shot_classifier(model, classnames, templates, args):
    with torch.no_grad():
        ...
        for classname in tqdm(classnames):
            texts = [
                _preprocess_text(template(classname)) for template in templates
            ]  # format with class
            texts = tokenize(texts, context_length=args.context_length).to(
                args.gpu
            )  # tokenize
...
if __name__ == "__main__":
    ...
    with open(
        os.path.join(args.save_dir, f"{args.dataset}.json"), "w", encoding="utf-8"
    ) as w:
        w.write(json_string)
