    ...
    with open(args.output, "w") as fout:
        with open(args.image_feats, "r") as fin:
            for line in tqdm(fin):
                ...
                score_tuples = []
                image_feat_tensor = torch.tensor(
                    [image_feat], dtype=torch.float
                ).cuda()  # [1, feature_dim]
                idx = 0
                while idx < len(text_ids):
                    text_feats_tensor = torch.from_numpy(
                        text_feats_array[
                            idx : min(idx + args.eval_batch_size, len(text_ids))
                        ]
                    ).cuda()  # [batch_size, feature_dim]
                    batch_scores = (
                        image_feat_tensor @ text_feats_tensor.t()
                    )  # [1, batch_size]
                ...
                fout.write(
                    "{}\n".format(
                        json.dumps(
                            {
                                "image_id": image_id,
                                "text_ids": [entry[0] for entry in top_k_predictions],
                            }
                        )
                    )
                )
