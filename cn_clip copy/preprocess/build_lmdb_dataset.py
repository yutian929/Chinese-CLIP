...
if __name__ == "__main__":
    ...
    for split in specified_splits:
        ...
        pairs_annotation_path = os.path.join(
            args.data_dir, "{}_texts.jsonl".format(split)
        )
        with open(pairs_annotation_path, "r", encoding="utf-8") as fin_pairs:
            ...
            for line in tqdm(fin_pairs):
                ...
                for image_id in obj["image_ids"]:
                    dump = pickle.dumps(
                        (image_id, obj["text_id"], obj["text"])
                    )  # encoded (image_id, text_id, text)
                    txn_pairs.put(
                        key="{}".format(write_idx).encode("utf-8"), value=dump
                    )
                    write_idx += 1
                    if write_idx % 5000 == 0:
                        txn_pairs.commit()
                        txn_pairs = env_pairs.begin(write=True)
            txn_pairs.put(
                key=b"num_samples", value="{}".format(write_idx).encode("utf-8")
            )
            txn_pairs.commit()
            env_pairs.close()
        print(
            "Finished serializing {} {} split pairs into {}.".format(
                write_idx, split, lmdb_pairs
            )
        )
        ...
        with open(base64_path, "r", encoding="utf-8") as fin_imgs:
            ...
            for line in tqdm(fin_imgs):
                ...
                txn_img.put(
                    key="{}".format(image_id).encode("utf-8"), value=b64.encode("utf-8")
                )
                write_idx += 1
                if write_idx % 1000 == 0:
                    txn_img.commit()
            ...
            env_img.close()
        print(
            "Finished serializing {} {} split images into {}.".format(
                write_idx, split, lmdb_img
            )
        )
