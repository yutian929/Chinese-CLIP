...
def load_from_name(
    ...
    if name in _MODELS:
        ...
        model_name, model_input_resolution = (
            _MODEL_INFO[name]["struct"],
            _MODEL_INFO[name]["input_resolution"],
        )
        ...
        model_path = name
        model_name, model_input_resolution = (
            f"{vision_model_name}@{text_model_name}",
            input_resolution,
        )
    else:
        raise RuntimeError(
...
def load(
    ...
    restore_model(model, clip_state_dict, bert_state_dict, use_flash_attention).to(
        device
    )
...
def tokenize(
    texts: Union[str, List[str]], context_length: int = 52
) -> torch.LongTensor:
    """
    ...
    for text in texts:
        all_tokens.append(
            ...
            + _tokenizer.convert_tokens_to_ids(_tokenizer.tokenize(text))[
                : context_length - 2
            ]
...
def create_model(model_name, checkpoint=None):
    ...
    vision_model_config_file = (
        Path(__file__).parent / f"model_configs/{vision_model.replace('/', '-')}.json"
    )
    print("Loading vision model config from", vision_model_config_file)
    assert os.path.exists(vision_model_config_file)

    text_model_config_file = (
        Path(__file__).parent / f"model_configs/{text_model.replace('/', '-')}.json"
    )
