...
class LMDBDataset(Dataset):
    def __init__(
        ...
        assert os.path.isdir(
            lmdb_path
        ), "The LMDB directory {} of {} split does not exist!".format(lmdb_path, split)
        lmdb_pairs = os.path.join(lmdb_path, "pairs")
        assert os.path.isdir(
            lmdb_pairs
        ), "The LMDB directory {} of {} image-text pairs does not exist!".format(
            lmdb_pairs, split
        )
        lmdb_imgs = os.path.join(lmdb_path, "imgs")
        assert os.path.isdir(
            lmdb_imgs
        ), "The LMDB directory {} of {} image base64 strings does not exist!".format(
            lmdb_imgs, split
        )
        ...
        self.number_samples = int(
            self.txn_pairs.get(key=b"num_samples").tobytes().decode("utf-8")
        )
        self.number_images = int(
            self.txn_imgs.get(key=b"num_images").tobytes().decode("utf-8")
        )
        logging.info(
            "{} LMDB file contains {} images and {} pairs.".format(
                split, self.number_images, self.number_samples
            )
        )
    ...
    def __getitem__(self, index):
        ...
        pair = pickle.loads(
            self.txn_pairs.get("{}".format(sample_index).encode("utf-8")).tobytes()
        )
        ...
        image = Image.open(
            BytesIO(base64.urlsafe_b64decode(image_b64))
        )  # already resized
        image = self.transform(image)

        text = tokenize(
            [_preprocess_text(raw_text)], context_length=self.max_txt_length
        )[0]
...
def fetch_resolution(vision_model):
    ...
    vision_model_config_file = (
        ...
        / f"clip/model_configs/{vision_model.replace('/', '-')}.json"
    )
...
def get_data(args, epoch_id=0, max_txt_length=64):
    ...
    if args.train_data:
        data["train"] = get_dataset(
            args, is_train=True, max_txt_length=max_txt_length, epoch_id=epoch_id
        )

    if args.val_data:
        data["val"] = get_dataset(
            args, is_train=False, max_txt_length=max_txt_length, epoch_id=epoch_id
        )
