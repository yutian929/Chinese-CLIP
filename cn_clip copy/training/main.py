...
from cn_clip.clip.model import (
    convert_weights,
    convert_state_dict,
    resize_pos_embed,
    CLIP,
...
def torch_version_str_compare_lessequal(version1, version2):
    ...
    assert (
        len(v1) == 3
    ), "Cannot parse the version of your installed pytorch! ({})".format(version1)
    assert (
        len(v2) == 3
    ), "Illegal version specification ({}). Should be in 1.X.Y format.".format(version2)
...
def main():
    ...
    vision_model_config_file = (
        ...
        / f"clip/model_configs/{args.vision_model.replace('/', '-')}.json"
    )
    ...
    text_model_config_file = (
        Path(__file__).parent.parent
        / f"clip/model_configs/{args.text_model.replace('/', '-')}.json"
    )
    ...
    if args.clip_weight_path is not None:
        assert os.path.exists(
            args.clip_weight_path
        ), "Pretrained CLIP weight not exists!"
    if args.bert_weight_path is not None:
        assert os.path.exists(
            args.bert_weight_path
        ), "Pretrained BERT weight not exists!"
    load(
        model,
    ...
    find_unused_parameters = torch_version_str_compare_lessequal(
        torch.__version__, "1.8.0"
    )
    ...
    if args.grad_checkpointing and not torch_version_str_compare_lessequal(
    ...
    if is_master(args):
        ...
        params_file = os.path.join(
            args.logs, args.name, "params_{}.txt".format(time_suffix)
        )
    ...
    if args.resume is not None:
        if os.path.isfile(args.resume):
            ...
            if not args.reset_data_offset:
                ...
                data = get_data(
                    args, epoch_id=start_epoch, max_txt_length=args.context_length
                )
    ...
    args.should_save = (
        args.logs is not None and args.logs != "" and args.logs.lower() != "none"
    ) and is_master(args)
    ...
    if args.distillation:
        try:
            ...
            raise ImportError(
                "modelscope is not installed. Please install it by `pip install modelscope`."
            )
        ...
        assert (
            args.teacher_model_name in teacher_model_dict
        ), "Error: Valid teacher model name has not been built."
        ...
        if "model" in mapping and hasattr(teacher_model, "model"):
            ...
            if hasattr(model_instance, mapping["model"]):
                setattr(
                    teacher_model,
                    "get_feature",
                    getattr(model_instance, mapping["model"]),
                )
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.distillation:
            ...
            num_steps_this_epoch = train(
                model, data, epoch, optimizer, scaler, scheduler, args, steps
            )
        ...
        if epoch + 1 < args.max_epochs:
            data = get_data(
                args, epoch_id=epoch + 1, max_txt_length=args.context_length
            )
