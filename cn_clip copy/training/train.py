...
def get_loss(
    ...
    if args.accum_freq == 1:
        ...
        if args.distillation:
            ...
            teacher_image_features = torch.cat(
                ...
                + [teacher_chunk_image_features]
                + teacher_accum_image_features[accum_idx + 1 :]
            )

        image_features = torch.cat(
            accum_image_features[:accum_idx]
            + [chunk_image_features]
            + accum_image_features[accum_idx + 1 :]
        )
        text_features = torch.cat(
            accum_text_features[:accum_idx]
            + [chunk_text_features]
            + accum_text_features[accum_idx + 1 :]
        )
    ...
    if args.aggregate:
        ...
        if args.gather_with_grad:
            ...
            if args.distillation:
                all_teacher_image_features = torch.cat(
                    torch.distributed.nn.all_gather(teacher_image_features), dim=0
                )
        else:
        ...
        if args.distillation:
            ...
            kd_loss = cosineSimilarityLoss(
                all_teacher_image_features, all_image_features
            )
    ...
    if args.report_training_batch_acc:
        i2t_acc = (logits_per_image.argmax(-1) == ground_truth).sum() / len(
            logits_per_image
        )
        t2i_acc = (logits_per_text.argmax(-1) == ground_truth).sum() / len(
            logits_per_text
        )
    ...
    for i in range(0, dataloader.num_batches):
        ...
        if is_master(args) and ((step + 1) % args.log_interval) == 0:
            ...
            logging.info(
                ...
                + (
                    f"Image2Text Acc: {acc['i2t'].item() * 100:.2f} | "
                    if args.report_training_batch_acc
                    else ""
                )
                + (
                    f"Text2Image Acc: {acc['t2i'].item() * 100:.2f} | "
                    if args.report_training_batch_acc
                    else ""
                )
        ...
        if (
            args.should_save
            and args.save_step_frequency > 0
            and ((step + 1) % args.save_step_frequency) == 0
        ):
            save_path = os.path.join(
                args.checkpoint_path, f"epoch_{epoch + 1}_{step + 1}.pt"
            )
            t1 = time.time()
            torch.save(
                {
...
def evaluate(model, data, epoch, args, steps):
    ...
    logging.info(
        "Begin to eval on validation set (epoch {} @ {} steps)...".format(
            epoch + 1, steps
        )
    )
    ...
    with torch.no_grad():
        for i in range(dataloader.num_batches):
            ...
            cumulative_i2t_acc += (
                (logits_per_image.argmax(-1) == ground_truth).sum()
            ).float()
            cumulative_t2i_acc += (
                (logits_per_text.argmax(-1) == ground_truth).sum().float()
            )

            if (i + 1) % 100 == 0:
                logging.info(
                    "Evaluated {}/{} batches...".format(i + 1, dataloader.num_batches)
                )
...
def cosineSimilarityLoss(feature1, feature2):
    ...
    feature2_interpolated = F.interpolate(
