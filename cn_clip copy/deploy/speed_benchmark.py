...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="The batch-size of the inference input. Default to 1.",
    )
    parser.add_argument(
        "--n",
        default=100,
        type=int,
        help="The iteration number for inference speed test. Default to 100.",
    )
    parser.add_argument(
        "--warmup", default=10, type=int, help="Warmup iterations. Default to 10."
    )
    ...
    parser.add_argument(
        ...
        choices=["cuda", "cpu"],
        default="cuda",
        help="CPU or GPU speed test. Default to cuda",
    )
    parser.add_argument(
        "--context-length",
        type=int,
...
if __name__ == "__main__":
    ...
    image = torch.vstack(
        [preprocess(Image.open("examples/pokemon.jpeg")).unsqueeze(0)] * args.batch_size
    )
    text = torch.vstack(
        [clip.tokenize(["杰尼龟"], context_length=args.context_length)] * args.batch_size
    )
    if args.device == "cuda":
        image = image.cuda()
        text = text.cuda()
    ...
    if args.pytorch_ckpt:
        ...
        for i in range(args.n):
            with track_infer_time(time_buffer):
                pytorch_output = pt_model(image=image, text=None)
        print_timings(
            name=f"Pytorch image inference speed (batch-size: {args.batch_size}):",
            timings=time_buffer,
        )
        del pt_model
    ...
    if args.onnx_image_model:
        ...
        session = onnxruntime.InferenceSession(
            args.onnx_image_model,
            sess_options=sess_options,
            providers=[onnx_execution_provider_map[args.device]],
        )
        for i in range(args.warmup):
            onnx_output = session.run(
                ["unnorm_image_features"], {"image": image.cpu().numpy()}
            )
        ...
        print_timings(
            name=f"ONNX image inference speed (batch-size: {args.batch_size}):",
            timings=time_buffer,
        )
        del session
    ...
    if args.tensorrt_image_model and args.device == "cuda":
        ...
        for i in range(args.n):
            with track_infer_time(time_buffer):
                trt_output = trt_model(inputs={"image": image})
        print_timings(
            name=f"TensorRT image inference speed (batch-size: {args.batch_size}):",
            timings=time_buffer,
        )
        del trt_model
    ...
    if args.pytorch_ckpt:
        ...
        for i in range(args.n):
            with track_infer_time(time_buffer):
                pytorch_output = pt_model(image=None, text=text)
        print_timings(
            name=f"Pytorch text inference speed (batch-size: {args.batch_size}):",
            timings=time_buffer,
        )
        del pt_model
    ...
    if args.onnx_text_model:
        ...
        run_options.log_severity_level = 2
        session = onnxruntime.InferenceSession(
            args.onnx_text_model,
            sess_options=sess_options,
            providers=[onnx_execution_provider_map[args.device]],
        )
        for i in range(args.warmup):
            onnx_output = session.run(
                ["unnorm_text_features"], {"text": text.cpu().numpy()}
            )
        ...
        print_timings(
            name=f"ONNX text inference speed (batch-size: {args.batch_size}):",
            timings=time_buffer,
        )
        del session
    ...
    if args.tensorrt_text_model and args.device == "cuda":
        ...
        for i in range(args.n):
            with track_infer_time(time_buffer):
                trt_output = trt_model(inputs={"text": text})
        print_timings(
            name=f"TensorRT text inference speed (batch-size: {args.batch_size}):",
            timings=time_buffer,
        )
        del trt_model
