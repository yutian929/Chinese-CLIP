...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.pytorch_ckpt_path):
        ...
        input_ckpt_path = _download(
            _MODELS[args.model_arch],
            args.download_root or os.path.expanduser("./cache/clip"),
        )
    else:
        raise RuntimeError(
    ...
    model = (
        create_model(_MODEL_INFO[args.model_arch]["struct"], checkpoint).float().eval()
    )
    ...
    if args.convert_text:
        ...
        text_fp16_onnx_model = convert_float_to_float16(
            text_fp32_onnx_model, keep_io_types=True, disable_shape_infer=True
        )
        save_model(
            text_fp16_onnx_model,
            text_fp16_onnx_path,
    ...
    if args.convert_vision:
        ...
        vision_fp16_onnx_model = convert_float_to_float16(
            vision_fp32_onnx_model, keep_io_types=True, disable_shape_infer=True
        )
        save_model(
            vision_fp16_onnx_model,
    ...
    if args.convert_vision:
        print(
            f">>> The vision FP32 ONNX model is saved at {vision_fp32_onnx_path}"
            + (
                f" with extra file {vision_fp32_onnx_path}.extra_file"
                if vision_fp32_onnx_hasextra
                else ""
            )
        )
        print(
