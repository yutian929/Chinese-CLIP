...
def parse_args():
    ...
    parser.add_argument(
        "--batch-size", default=1, type=int, help="The batch size of the TensorRT model"
    )
    parser.add_argument(
        "--context-length",
        type=int,
        default=52,
        help="The padded length of input text (include [CLS] & [SEP] tokens).",
    )
    parser.add_argument(
        "--save-tensorrt-path",
        required=True,
...
if __name__ == "__main__":
    ...
    if args.convert_text:
        ...
        assert os.path.exists(
            input_text_onnx_path
        ), f"Error: The specified --text-onnx-path {input_text_onnx_path} not exists!"
    ...
    if args.convert_vision:
        ...
        assert os.path.exists(
            input_vision_onnx_path
        ), f"Error: The specified --vision-onnx-path {input_vision_onnx_path} not exists!"
    ...
    if args.fp32:
        ...
        if args.convert_vision:
            ...
            print(
                f"Saving the vision FP32 TensorRT model at {vision_fp32_trt_path} ..."
            )
    ...
    if args.fp16:
        ...
        if args.convert_vision:
            ...
            print(
                f"Saving the vision FP16 TensorRT model at {vision_fp16_trt_path} ..."
            )
    ...
    if args.convert_vision:
        if args.fp32:
            print(
                f">>> The vision FP32 TensorRT model is saved at {vision_fp32_trt_path}"
            )
        if args.fp16:
            print(
                f">>> The vision FP16 TensorRT model is saved at {vision_fp16_trt_path}"
            )
