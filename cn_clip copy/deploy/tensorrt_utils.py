...
class TensorRTShape:
    ...
    * maximum input shape
    ...
    def check_validity(self) -> None:
        ...
        assert (
            self.min_shape[0] > 0
            and self.optimal_shape[0] > 0
            and self.max_shape[0] > 0
        )
...
def fix_fp16_network(
    network_definition: INetworkDefinition, fp16_banned_ops=None
) -> INetworkDefinition:
    """
    ...
    **TIP**: minimum batch size should be 1 in most cases.
    ...
    if workspace_size is not None:
        config.set_memory_pool_limit(
            trt.tensorrt.MemoryPoolType.DLA_GLOBAL_DRAM, workspace_size
        )
    config.set_tactic_sources(
        ...
        )  # trt advised to use cuDNN for transfo architecture
    )
    if int8:
    ...
    if len(input_shapes) == 1 and input_shapes[0].input_name is None:
        names = [
            network_def.get_input(num_input).name
            for num_input in range(network_def.num_inputs)
        ]
    ...
    logger.log(
        msg="building engine. depending on model size this may take a while",
        severity=trt.ILogger.WARNING,
    )
    start = time()
    ...
    logger.log(
        msg=f"building engine took {time() - start:4.1f} seconds",
        severity=trt.ILogger.WARNING,
    )
    assert (
        engine is not None
...
class TensorRTModel(object):
    def __init__(self, engine_path):
        ...
        self.input_binding_idxs, self.output_binding_idxs = get_binding_idxs(
            self.engine, profile_index
        )
    ...
    def __call__(self, inputs, time_buffer=None):
        ...
        for i in range(self.context.engine.num_bindings):
            ...
            assert isinstance(
                tensor, torch.Tensor
            ), f"unexpected tensor class: {type(tensor)}"
            assert (
