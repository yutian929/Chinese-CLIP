    ...
    if f"{prefix}visual.transformer.resblocks.0.attn.in_proj_weight" in state_dict:
        for k in list(state_dict.keys()):
            if "attn.in_proj_weight" in k:
                state_dict[
                    k.replace("attn.in_proj_weight", "attn.Wqkv.weight")
                ] = state_dict.pop(k)
            elif "attn.in_proj_bias" in k:
                state_dict[
                    k.replace("attn.in_proj_bias", "attn.Wqkv.bias")
                ] = state_dict.pop(k)
    elif f"{prefix}visual.transformer.resblocks.0.attn.Wqkv.weight" in state_dict:
        for k in list(state_dict.keys()):
            if "attn.Wqkv.weight" in k:
                state_dict[
                    k.replace("attn.Wqkv.weight", "attn.in_proj_weight")
                ] = state_dict.pop(k)
            elif "attn.Wqkv.bias" in k:
                state_dict[
                    k.replace("attn.Wqkv.bias", "attn.in_proj_bias")
                ] = state_dict.pop(k)
...
class HeatCLIPVisualTransformer(nn.Module):
    def __init__(
        self,
        input_resolution: int,
        patch_size: int,
...
if __name__ == "__main__":
    ...
    model = HeatCLIPVisualTransformer(
        input_resolution, patch_size, width, layers, heads, output_dim
    ).to(device)

    batch_size = 32
    random_input = torch.randn(batch_size, 4, input_resolution, input_resolution).to(
        device
    )
    with torch.no_grad():
